Standup.script :node do
  ...
  def run
    ...
    with_processed_file script_file('unicorn') do |file|
      upload file,
    ...
    with_processed_file script_file('webapp.conf') do |file|
      scripts.nginx.add_server_conf file, "#{params.name}.conf"
    end

    upload script_file('htpasswd'),
           :to => '/opt/',
           :sudo => true

    scripts.monit.add_watch script_file("unicorn_monit.conf")

    params.workers.times do |n|
      @worker_num = n
      with_processed_file script_file('unicorn_worker_monit.conf') do |file|
        scripts.monit.add_watch file, "unicorn_worker_#{n}_monit.conf"
      end
    end

    with_processed_file script_file('unicorn.conf.rb') do |file|
      upload file,
             :to => File.join(app_path, 'config'),
             :sudo => true
    end
  end
  ...
  def restart
    in_dir app_path do
      sudo 'monit restart -g unicorn'
      sudo 'monit restart -g unicorn_workers'
